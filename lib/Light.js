...
var Q = require('q');
var debug = require('debug')('insteon-api.light');
...
Light.prototype.onLevel = function (level, next) {
...
Light.prototype.dataHandler = function (data, next) {
  ...
  case 'off':
    this.emit('turnOff');
      deferred.resolve(['turnOff']);
    break;
  case 'fast_on':
    this.emit('turnOnFast');
      deferred.resolve(['turnOnFast']);
    break;
  case 'fast_off':
    this.emit('turnOffFast');
      deferred.resolve(['turnOffFast']);
    break;
  case 'unknown':
    this.emit('change');
