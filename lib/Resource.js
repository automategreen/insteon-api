...
function Resource(api, type, properties, data) {
  ...
  for(var prop in properties) {
    var value = data[prop] || data[properties[prop].name];

    if(properties[prop].elements && Array.isArray(value)) {
      value = parseElements(properties[prop].elements, value);
    }
...
function parseElements(elements, value) {
  return value.map(function(e) {
    var o = {};
    for(var eProp in elements) {
      o[eProp] = e[eProp] || e[elements[eProp].name];
    }
    return o;
  });
}
